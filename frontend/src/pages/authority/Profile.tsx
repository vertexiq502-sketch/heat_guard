...
const INPUT = 'w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-purple-500 focus:border-purple-500 sm:text-sm';
    ...
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex items-center gap-4">
          <div className="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center text-purple-800 text-2xl font-bold uppercase shadow-sm">
            {profile.name?.charAt(0) || 'A'}
          </div>
          <div>
            <h2 className="text-xl font-bold text-gray-900">{profile.name || 'Authority'}</h2>
            <p className="text-gray-500 text-sm capitalize">{t('role_authority')}</p>
          </div>
        </div>
      ...
      {isEditing && (
        <div className="flex gap-4 justify-end">
          ...
          <button
            onClick={handleSave}
            disabled={isPending}
            className="px-6 py-2 bg-purple-600 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white hover:bg-purple-700 disabled:opacity-50"
          >
